...
class Show{
    ...
    // ***************************************************
    Drone* betterInsert(Drone* root, const Drone& aDrone); // helper function to insert
    Drone* betterRemove(Drone* root, int id); // helper function to remove
    Drone *rotateLeft(Drone* x); // helper function to rotate
    Drone *rotateRight(Drone *y); // helper functon to rotate
    Drone *findLightOffDrone();
    Drone *betterFindDrone(int id);
    int getBalance(Drone* aDrone); // helper function to know if re-balancing needs to be done
    int height(Drone* aDrone); // helper function to get height from nullptr
    int max(int a, int b); // helper function to get learger number
    void betterClear(Drone* aDrone); // helper function for clearing tree
    void copyTree(Drone*& lhs, Drone* rhs); // helper function to copy rhs tree
};
#endif