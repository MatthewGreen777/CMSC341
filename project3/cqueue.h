...
class CQueue{
    ...
    CQueue(prifn_t priFn, HEAPTYPE heapType, STRUCTURE structure); // done
    ~CQueue(); // done
    CQueue(const CQueue& rhs); // done
    CQueue& operator=(const CQueue& rhs);
    void insertOrder(const Order& order); // done
    Order getNextOrder(); // Return the highest priority order // done
    void mergeWithQueue(CQueue& rhs); // done
    void clear(); // done
    int numOrders() const; // Return number of orders in queue // done
    void printOrdersQueue() const; // Print the queue using preorder traversal // done
    prifn_t getPriorityFn() const; // done
    // Set a new priority function. Must rebuild the heap!!!
    void setPriorityFn(prifn_t priFn, HEAPTYPE heapType); // done
    HEAPTYPE getHeapType() const; // done
    STRUCTURE getStructure() const; // done
    // Set a new data structure (skew/leftist). Must rebuild the heap!!!
    void setStructure(STRUCTURE structure); // done
     ...
     ******************************************/
    Node *mergeLeftistQueues(Node *node, Node *node2); // done
    Node *mergeSkewQueues(Node *node, Node *node2); // done
    Node *mergeLeftHelp(Node *node, Node *node2); // done
    Node *max(Node *a, Node *b); // helper function to get larger number
    Node *min(Node *a, Node *b); // helper function to get smaller number
    Node *helpDeepCopy(Node *node); // done
    int getPriority(Node *node) const; // done

    void swap(Node *node); // done
    void reBuildHeap(Node* node); // done
    void betterClear(Node* node); // helper function for clearing tree
};
#endif